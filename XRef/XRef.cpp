...
void XRef::logWord( std::string const& word, size_t const lineno ) {
	...
	if ( entry != references_.end () ) { updateEntry ( entry, lineno ); } else { createEntry ( word, lineno ); }
}

void XRef::createEntry( std::string const& word, size_t const lineno ) { references_[word][lineno] = default_count_; }

void XRef::updateEntry( std::map<string, map<size_t, size_t>>::iterator it, size_t const& lineno ) const {
	...
	if ( entry_val == it->second.end () ) { it->second[lineno] = default_count_; } else { entry_val->second++; }
}

auto XRef::display( ostream& os ) -> void {
	for_each ( references_.begin (), references_.end (), [&]( auto& p ) {
		this->header ( os, p.first, longest_key_ );
